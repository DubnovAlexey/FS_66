...
export function createBasePromptByRole(user) {
    ...
    if (user.role === ROLES.GUEST) {
        ...
        Ты - квалифицированный повар, определяющий ингредиенты
        блюда по названию блюда.
        Тебе даётся только название желаемого блюда, без данных о
        содержимом какого-либо холодильника.
        Твоя задача - назвать основные продукты, необходимые для
        приготовления этого блюда.
        Правила:
            -возвращай только список продуктов.
            -не возвращай пояснения сверх списка продуктов.
        `;
    }
...
export function createPrompt(basePrompt, dishTitle, products = []) {
    ...
    const productText = products.length > 0
        ...
        : 'нет данных (для этой роли доступ к холодильнику не требуется)';
